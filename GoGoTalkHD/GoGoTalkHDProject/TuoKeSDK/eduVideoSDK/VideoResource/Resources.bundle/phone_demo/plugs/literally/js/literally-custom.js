...
CustomLiterally.prototype = {
    ...
    lcInit:function(ele , cloneElement , operationUserThirdid ,  initConfig , isDefalutClear , isDefalutDrawingChangeEvent){
        ...
        this.lc = window.LC.init(this.ele[0],this.initConfig);
...
//          	console.error("shapeSaveEvent" ,parameter);
                var shapeData  = window.LC.shapeToJSON(parameter.shape);
...
//          	console.error("redoEvent" ,parameter) ;
            	if( parameter.action.constructor.name === "AddShapeAction" ){
	                var shapeData  = window.LC.shapeToJSON(parameter.action.shape);
	                if(shapeData!=null && shapeData.className != null && (shapeData.className == "LinePath" || shapeData.className == "ErasedLinePath" )){
	                    shapeData.data.smoothedPointCoordinatePairs = null ;
	                    delete shapeData.data.smoothedPointCoordinatePairs;
	                    var tmpData = shapeData.data.pointCoordinatePairs ;
...
//          	console.error("laserMarkEvent" ,parameter);
    ...
    setBackgroundWatermarkImage:function(imgUrl){
        ...
        if(  imgUrl=="" || imgUrl==null || imgUrl==undefined ){
            ...
            backImg.src = "./plugs/literally/lib/img/transparent_bg.png?ts"+new Date().getTime();
            that.backImageResize(that,that.lcLitellyScalc,backImg,true,false);
    ...
    backImageResize:function(that,imgScale,backImg,isChangeCanvas,isChangeWatermarkScale){
        setTimeout(function () {
            ...
            if( $literrallyLayout.height() * imgScale < $literrallyLayout.width()  ){
                $bigLiterallyWrap.add($literally).height( Math.round( $literrallyLayout.height() )  * that.eleWHPercent);
                $bigLiterallyWrap.add($literally).width( Math.round(  $literrallyLayout.height()*imgScale ) * that.eleWHPercent );
                if(MINWH !=undefined && MINWH!=null){
                    $literally.add($bigLiterallyWrap).css({"min-width":Math.round(MINWH*imgScale)+"px"});
                    $literally.add($bigLiterallyWrap).css({"min-hegiht":MINWH+"px"});
                }
                $bigLiterallyWrap.css({"margin-top":"0px" , "margin-left":-$bigLiterallyWrap.width()/2+"px" , "top":"0px" , "left":"50%" });
            }else {
                $bigLiterallyWrap.add($literally).height( Math.round( $literrallyLayout.width()/imgScale ) * that.eleWHPercent );
                $bigLiterallyWrap.add($literally).width( $literrallyLayout.width() * that.eleWHPercent );
                if(MINWH !=undefined && MINWH!=null){
                    $literally.add($bigLiterallyWrap).css({"min-width":Math.round(MINWH*imgScale)+"px"});
                    $literally.add($bigLiterallyWrap).css({"min-height":MINWH+"px"});
                }
                $bigLiterallyWrap.css({"margin-left":"0px" , "margin-top":-$bigLiterallyWrap.height()/2+"px" , "left":"0px" , "top":"50%"  });
            }
            if(isChangeCanvas){
            ...
            // }
            $("#aynamic_ppt").height( $literally.height() ).width($literally.width());
        },0);
    },
    ...
    cloneElementScalc:function(clonePackElement){
        var thisObj =  window.GLOBAL.thisObj ;
        $(window).resize(function(){
            var bigLitCv =  thisObj.ele;
            var cloneImgLitCv = thisObj.cloneCanvasEle ;
            var smallLitCv = clonePackElement ;
            var tmpH = smallLitCv.height() / bigLitCv.height() ;
            var tmpX = smallLitCv.width() / bigLitCv.width() ;

            cloneImgLitCv.width(bigLitCv.width()).height(bigLitCv.height());
            cloneImgLitCv.css({
                '-webkit-transform':'scaleX('+tmpX+') scaleY('+tmpH+')' ,
                '-moz-transform':'scaleX('+tmpX+') scaleY('+tmpH+')' ,
                '-o-transform':'scaleX('+tmpX+') scaleY('+tmpH+')' ,
                '-ms-transform':'scaleX('+tmpX+') scaleY('+tmpH+')' ,
                'transform':'scaleX('+tmpX+') scaleY('+tmpH+')'
            });
            cloneImgLitCv.css({"left":"-"+( ( bigLitCv.width() - smallLitCv.width() )/2  )+"px","top":"-"+ ( ( bigLitCv.height()-smallLitCv.height() )/2 )+"px"});
        });
        $(window).resize();
    } ,
            ...
            tools:[
                {
                    ...
                    tool: new window.LC.tools.Pencil(this.lc)
                },{
                    name: 'pencilPhone',
                ...
                if(tl.ele.getAttribute("id") === thisObj.toolElement['tool_laser'] ){
                	...
                	if( thisObj.rolePermission["laser"] ){
                		...
                		$temporaryDrawPermission.on("mousemove",function(e){
	                		...
	                		var left =  (e.pageX - offset.left) /  $(this).width() *100;
	                		var top =   (e.pageY  - offset.top) /  $(this).height() *100;
                			$laserMark.css({
            					"left":left+"%" ,
            					"top":top+"%"
            				});
...
//      $(toolsConfig.tools[0]).trigger("click");
        ...
        toolsConfig.zoom.forEach(function(zm) {
            $(zm.ele).click(function(e) {
                ...
                $(window).trigger("resize");
//              if(thisObj.eleWHPercent==1){
//                  thisObj.ele.parents(thisObj.otherElement.bigLiterallyWrap).css({
//                      "width":(thisObj.eleWHPercent*100)+"%" ,
//                      "height":(thisObj.eleWHPercent*100)+"%" ,
//                  });
//					$(window).trigger("resize");
//              }else{
//                  thisObj.ele.parents(thisObj.otherElement.bigLiterallyWrap).css({
//                      "width":(thisObj.eleWHPercent*100)+"%" ,
//                      "height":(thisObj.eleWHPercent*100)+"%" ,
//                      "width":"calc( "+(thisObj.eleWHPercent*100)+"% + 1.05rem )" ,
//                      "height":"calc( "+(thisObj.eleWHPercent*100)+"%  + 1.05rem )" ,
//                  });
//					$(window).trigger("resize");
//              }
            });
...
//		var canvas = window.LC.renderShapesToCanvas(
//		  window.LC.snapshotJSONToShapes(snapshotJSON),

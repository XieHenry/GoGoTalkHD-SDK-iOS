...
@implementation GGT_PreTeachEvaView

- (instancetype)initWithFrame:(CGRect)frame model:(GGT_CourseCellModel *)model
{
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = UICOLOR_FROM_HEX(ColorF2F2F2);
        [self configViewWithModel:model];
    }
    return self;
}

- (void)configViewWithModel:(GGT_CourseCellModel *)model
{
#pragma mark - 整体
    // 整体
...
#pragma mark -
    ...
    self.xc_topStarView = ({
        XCStarView *xc_starView = [[XCStarView alloc] initWithEmptyImage:@"dianping_kebiao_da_wei" StarImage:@"dianping_kebiao_da_yi" totalStarCount:5 selectedStatCount:model.StuScore starMargin:LineW(11) starWidth:LineW(11)];
        xc_starView;
    });
    ...
    self.xc_topContentLabel = ({
        ...
        label.frame = CGRectMake(0, 0, self.width-100, 0);
        if ([model.Remark isKindOfClass:[NSString class]] && model.Remark.length > 0) {
            label.text = model.Remark;
        } else {
            label.text = @"";
        }
        [label changeLineSpaceWithSpace:5.0];
        [label sizeToFit];
        label;
    });
    ...
    [self.xc_topContentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@(self.xc_topContentLabel.height));
    }];
    ...
    [self.xc_evaParentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.xc_topParentView.mas_bottom);
    }];
    
    [self layoutIfNeeded];
    
    self.frame = CGRectMake(self.x, self.y, self.width, self.xc_evaParentView.bottom);
